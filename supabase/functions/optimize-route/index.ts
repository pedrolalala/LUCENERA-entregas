...
serve(async (req) => {
  ...
  try {
    ...
    if (!deliveries || deliveries.length === 0) {
      return new Response(JSON.stringify({ error: 'No deliveries provided' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const deliveriesInfo = deliveries
      .map(
        (d, i) =>
...
RETORNE EXATAMENTE este formato JSON (sem markdown, sem texto extra):
{
  ...
  "justificativa": "[Breve explicação da lógica de otimização]"
    ...
    optimizedRoute.rota_otimizada = optimizedRoute.rota_otimizada.map(
