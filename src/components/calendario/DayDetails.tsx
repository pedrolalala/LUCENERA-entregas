...
export function DayDetails({
  ...
  if (!dayData || dayData.total === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-4">Nenhuma entrega agendada para este dia</p>
      </div>
    )
  }
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs font-medium text-muted-foreground">#{entrega.codigo_obra}</span>
        </div>

        {/* Client name */}
        <h4 className="font-semibold text-foreground mb-2 line-clamp-1">{entrega.cliente}</h4>
  ...
  const renderGroup = (
    ...
    return (
      <div>
        ...
        <div className="space-y-3">{entregas.map((e, i) => renderEntregaCard(e, i))}</div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-primary capitalize">{formattedDate}</h3>
